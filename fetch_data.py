...
def fetch_rss(url, timeout=10):
    try:
        ...
        print("RSS fetch failed " + url + " : " + str(e))
        return ""
...
def fetch_fear_greed():
    try:
        ...
        print("Fear and Greed error: " + str(e))
...
for feed_url in RSS_FEEDS:
    ...
    if xml:
        ...
        print("    -> " + str(len(items)) + " articles")
    time.sleep(0.5)

print("Total articles: " + str(len(all_items)))
...
for item in all_items:
    ...
    for exchange, keywords in EXCHANGES.items():
        if has_mention(full_text, keywords):
...
sov_pct = {}
for ex, cnt in sov_counts.items():
    sov_pct[ex] = round(cnt / total_mentions * 100, 1)

for ex, cnt in sorted(sov_counts.items(), key=lambda x: -x[1]):
    print("  " + ex + ": " + str(cnt) + " mentions (" + str(sov_pct.get(ex, 0)) + "%)")
...
print("Done at " + output["generated_at"])
