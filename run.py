...
def generate_html(output):
    ...
    for ex in sorted(exchanges, key=lambda x: -exchanges[x]["sov"]):
        ...
        if delta is None:
            ...
            delta_html = '<span class="sd up">+{}% WoW</span>'.format(delta)
        else:
            delta_html = '<span class="sd down">{}% WoW</span>'.format(delta)
        s = d["sentiment"]
        cards_html += """
        <div class="sc" style="--accent:{color}">
          <div class="sl">{ex}</div>
          <div class="sv" style="color:{color}">{sov}%</div>
          {delta}
          <div class="sm">{mentions} mentions</div>
          <div class="sent">
            ...
            color=color, ex=ex, sov=d["sov"], delta=delta_html,
            mentions=d["mentions"], pos=s["positive"], neu=s["neutral"], neg=s["negative"]
        )
    ...
    for ex in sorted(exchanges, key=lambda x: -exchanges[x]["sov"]):
        ...
        if delta is None:
            ...
            delta_str = '<span class="up">+{}%</span>'.format(delta)
        else:
            delta_str = '<span class="down">{}%</span>'.format(delta)
        s = d["sentiment"]
        table_rows += """
          <tr>
            ...
            <td><b>{sov}%</b></td>
            <td>{delta}</td>
            <td><div class="bb"><div class="bf" style="width:{sov}%;background:{color}"></div></div></td>
            <td>
              <span class="badge pos-b">{pos}</span>
              <span class="badge neu-b">{neu}</span>
              <span class="badge neg-b">{neg}</span>
            </td>
          </tr>""".format(
            color=color, ex=ex, mentions=d["mentions"], sov=d["sov"],
            delta=delta_str, pos=s["positive"], neu=s["neutral"], neg=s["negative"]
        )
    ...
    for a in articles[:80]:
        ...
        art_rows += """
          <tr>
            ...
            color=color, ex=a["exchange"], link=a["link"],
            title=a["title"], sent=sent, pub=pub
        )
...
<header>
  <div class="logo">
    <div class="li">B</div>
    <div class="lt"><h1>Bitget PR Monitor</h1><p>Daily coverage and Share of Voice</p></div>
  </div>
  <div class="hr">
  ...
  <div class="mg">
    <div>
      <div class="pn">
        ...
        <table>
          <thead><tr><th>Exchange</th><th>Mentions</th><th>SOV</th><th>WoW</th><th>Share</th><th>Sentiment</th></tr></thead>
          <tbody>{table_rows}</tbody>
        </table>
        <div class="src">CoinTelegraph · CoinDesk · Decrypt · Blockworks · The Block · Messari · BeInCrypto · CryptoSlate · Watcher.Guru · Protos · The Defiant · Bitcoinist · AmbCrypto + Google News</div>
      </div>
      <div class="pn">
        <div class="ph">Recent Coverage <span class="ct">{art_count} articles</span></div>
        <table>
          <thead><tr><th>Exchange</th><th>Headline</th><th>Sentiment</th><th>Date</th></tr></thead>
    ...
    <div>
      <div class="pn">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr><td>Exchanges tracked</td><td><b>{ex_count}</b></td></tr>
      ...
      <div class="pn">
        ...
        <table>
          ...
          <tbody>
            <tr><td>Positive</td><td><b style="color:#00e676">{bg_pos}</b></td></tr>
            <tr><td>Neutral</td><td><b style="color:var(--mu)">{bg_neu}</b></td></tr>
            <tr><td>Negative</td><td><b style="color:#ff4d6d">{bg_neg}</b></td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
<script>
  new Chart(document.getElementById("sovChart").getContext("2d"), {{
    type: "bar",
    data: {{
      labels: {ex_order_json},
      datasets: [{{ label: "SOV %", data: {sov_json}, backgroundColor: {colors_json}, borderRadius: 6 }}]
    }},
    options: {{
      ...
      scales: {{
        ...
        art_rows=art_rows,
        art_count=min(80, len(articles)),
        bg_pos=exchanges["Bitget"]["sentiment"]["positive"],
        bg_neu=exchanges["Bitget"]["sentiment"]["neutral"],
        bg_neg=exchanges["Bitget"]["sentiment"]["negative"],
        ex_order_json=json.dumps(ex_order),
        sov_json=json.dumps(sov_values),
...
def main():
    ...
    print("\n[1/2] Fetching {} Google News feeds...".format(len(GOOGLE_FEEDS)))
    ...
    print("\n[2/2] Fetching {} direct media feeds...".format(len(DIRECT_FEEDS)))
    ...
    for ex, d in sorted(exchange_data.items(), key=lambda x: -x[1]["sov"]):
        delta = d["sov_delta_wow"]
        dstr  = "({}{}% WoW)".format("+" if delta and delta >= 0 else "", delta) if delta is not None else "(first run)"
        s     = d["sentiment"]
        print("  {:8s}: {:4d} mentions | SOV {:5.1f}% {} | +{} ~{} -{}".format(
            ex, d["mentions"], d["sov"], dstr, s["positive"], s["neutral"], s["negative"]
        ))
