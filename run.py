...
"<div class='cd mo'><div class='ch'>"
    ...
    "<script>"
    "// Dynamic data fetching - runs on page load"
    ...
    "    "
    "    // Update timestamp"
    "    document.querySelector('.ts').textContent = 'Updated: ' + data.generated_at;"
    "    "
    "    // Update stat cards"
    "    const stats = ["
    "      [data.reserve_btc, data.reserve_yoy, 'up'],"
    "      [data.reserve_ratio, 'Overcollateralized', 'up'],"
    "      [data.net_inflows, 'Strong', 'up'],"
    "      ['$' + data.btc_price, data.btc_change + '% 24h', data.btc_change >= 0 ? 'up' : 'down'],"
    "      [data.fg_value, data.fg_class, 'neutral'],"
    "      [data.sov_pct.Bitget + '%', data.sov_counts.Bitget + ' mentions', 'neutral']"
    "    ];"
    "    const cards = document.querySelectorAll('.sc');"
    "    stats.forEach((s,i) => { if(cards[i]) { cards[i].querySelector('.sv').textContent = s[0]; cards[i].querySelector('.sd').textContent = s[1]; cards[i].querySelector('.sd').className = 'sd ' + s[2]; }});"
    "    "
    "    // Update SOV table"
    "    const order = ['Bitget','Binance','OKX','Bybit','MEXC','KuCoin'];"
    "    const colors = {'Bitget':'#00c4ff','Binance':'#7b61ff','OKX':'#ff4d6d','Bybit':'#ff9800','MEXC':'#ffd740','KuCoin':'#00e676'};"
    "    let rows = '';"
    "    order.forEach(ex => { rows += '<tr><td>'+ex+'</td><td>'+(data.sov_counts[ex]||0)+'</td><td>'+(data.sov_pct[ex]||0)+'%</td><td><div class=bb><div class=bf style=width:'+(data.sov_pct[ex]||0)+'%;background:'+colors[ex]+'></div></div></td></tr>'; });"
    "    document.querySelector('.st tbody').innerHTML = rows;"
    "    "
    "    // Update chart"
    ...
    "    "
    "    // Update Bitget news"
    "    const newsContainer = document.querySelectorAll('.pn')[1];"
    "    if(newsContainer && data.bitget_news && data.bitget_news.length > 0) {"
    "      let newsHtml = '';"
    "      data.bitget_news.forEach(n => { newsHtml += '<div class=ni><div class=nm><span class=nd>'+n.pub+'</span><span class=ns>'+n.source+'</span></div><div class=nt>'+n.title+'</div><a class=nl href='+n.link+' target=_blank>↗ Read</a></div>'; });"
